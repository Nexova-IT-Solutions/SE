  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-slate-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-3xl flex-col gap-6">
        <div>
          <h1 className="text-3xl font-bold text-[#1F1720]">Account Settings</h1>
          <p className="mt-1 text-sm text-[#6B5A64]">Manage admin account preferences and security settings.</p>
        </div>

        <Card className="rounded-2xl border border-brand-border bg-white shadow-sm">
          <CardHeader>
            <CardTitle className="text-[#1F1720]">Account settings</CardTitle>
            <CardDescription>Connect the account settings workflow for {session.user.name || session.user.email || "this user"} here.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-3">
            ...
            <Button asChild variant="outline" className="rounded-xl border-brand-border text-[#1F1720] hover:bg-[#FDF9E8] hover:text-[#315243]">
              <Link href={`/${locale}/profile/settings`}>Open profile settings</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
